...
| `api2.realtor.ca/Location.svc/*`, `api2.realtor.ca/Listing.svc/PropertySearch_Post` | Imperva (reese84) | native-TLS (no browser) | 2026-06-06: This host fingerprints the **TLS stack**. Every wreq profile (Chrome/Safari/OkHttp, H1 or H2, with or without valid `visid_incap`/`nlbi`/`incap_ses` cookies) gets the reese84 challenge; curl/Python-urllib (system OpenSSL) **without** `Sec-Fetch-*` gets 200 + JSON. wafer now auto-falls-back to a urllib/OpenSSL transport on Imperva detection (see `_native_tls.py`), pinned per-host. Full search flow verified no-browser: SubAreaSearch→PropertySearch_Post→pagination, 200 every call. **Don't rapid-fire** -volume trips a rate-based reese84 page even on OpenSSL (recovers after cooldown; sticky path backs off and retries). |
