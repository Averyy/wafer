...
| `support.lutron.com/us/en/product/casetawireless/article/product-selection/Diva-Smart-Dimmer` | Imperva (reese84 interstitial) | browser-solve | 2026-08-02: Serves the reese interstitial as **HTTP 200** (~6.2KB) on a cold request; native-TLS/OpenSSL gets the same interstitial, so it falls through to browser-solve. Regression site for the **embedder-vs-direct-nav** fix: this is a non-www *content* host, and the old "non-www subdomain -> `https://www.<reg>/`" heuristic sent the solve to `www.lutron.com`, which is a **different Imperva site** (`support` = 2477703 via `visid_incap_2477703`, `www` = 1555970 via `nlbi_1555970`) that never challenges. The solve logged `solved=True (20 cookies)` and the replay stayed challenged forever. Now the interstitial body short-circuits the derivation to direct navigation: `session.get()` -> 200 **110,875 bytes** of real article in ~5s (browser passthrough, 14 cookies injected). See `docs/ref-imperva.md`. |
